...
export class FocusReader implements BaseReader {
    ...
    public applyStyles() {
        const displayMode = this.settings.focus?.displayMode || 'Margin';
        const fixationSettings = this.settings.focus?.fixation || { width: 600, height: 300 };

        if (displayMode === 'Fixation') {
            const fixationChunkSize = (fixationSettings as any).chunkSize || 1;
            this.settings.chunkSize = fixationChunkSize;
            if (this.onChunkSizeChange) {
                this.onChunkSizeChange(fixationChunkSize);
            }
        }
        ...
        if (displayMode === 'Fixation') {
            this.displayElement.style.width = `${fixationSettings.width}px`;
            this.displayElement.style.height = `${fixationSettings.height}px`;
            this.displayElement.style.minWidth = '300px';
            this.displayElement.style.maxWidth = '800px';
            this.displayElement.style.minHeight = '60px';
        } else {
            const marginSettings = this.settings.focus?.margin || { 
                width: 600, 
                height: 300
            };
            this.displayElement.style.width = `${marginSettings.width}px`;
            this.displayElement.style.height = `${marginSettings.height}px`;
    ...
    public updateSettings(settings: SpeedReaderSettings): void {
        ...
        const displayMode = settings.focus?.displayMode || 'Margin';
        const fixationSettings = settings.focus?.fixation || { chunkSize: 1 };

        if (displayMode === 'Fixation') {
            settings.chunkSize = fixationSettings.chunkSize;
            if (this.onChunkSizeChange) {
                this.onChunkSizeChange(fixationSettings.chunkSize);
            }
        }
    ...
    private _appendSplitWordSegment(parent: HTMLElement, text: string, activeColor: string, inactiveColor: string) {
        if (text.length === 1) {
        ...
        if (part1Text) {
            ...
            part1.textContent = part1Text;
            part1.style.color = activeColor;
            parent.appendChild(part1);
        }
        
        if (part2Text) {
            const part2 = document.createElement('span');
            part2.textContent = part2Text;
            part2.style.color = inactiveColor;
            parent.appendChild(part2);
        }
    }
    ...
    private createLineElement(lineIndex: number): HTMLElement {
        ...
        wordsInLine.forEach((word, index) => {
            ...
            if (displayMode === 'Fixation') {
                wordSpan.innerHTML = '';
                const activeColor = this.settings.activeTextColor || 'inherit';
                const inactiveColor = this.settings.inactiveTextColor || 'inherit';
                const punctuationRegex = new RegExp(/[.,\/;\-:]/);

                if (word.length === 1) {
                    const span = document.createElement('span');
                    span.textContent = word;
                    span.style.color = punctuationRegex.test(word) ? inactiveColor : activeColor;
                    ...
                    for (let i = 0; i < word.length; i++) {
                        ...
                        if (isPunctuation) {
                            if (currentSegment.length > 0) {
                                this._appendSplitWordSegment(wordSpan, currentSegment, activeColor, inactiveColor);
                                currentSegment = '';
                            }
                            const puncSpan = document.createElement('span');
                            puncSpan.textContent = char;
                            puncSpan.style.color = inactiveColor;
                            wordSpan.appendChild(puncSpan);
                        } else {
                            currentSegment += char;
                        }
                    }
    ...
    private highlightCurrentWords() {
        ...
        if (displayMode === 'Margin') {
            ...
            const marginPercentage = (this.settings.focus?.margin?.percentage || 10) / 100;
            const leftMarginEnd = containerRect.width * marginPercentage;
            const rightMarginStart = containerRect.width * (1 - marginPercentage);

            const activeColor = this.settings.activeTextColor || 'inherit';
            const inactiveColor = this.settings.inactiveTextColor || 'inherit';

            if (this.visibleLines.length === 0 || this.lineWordMappings.length === 0) return;
            
            const firstVisibleLineMapping = this.lineWordMappings[this.visibleStartIndex];
            const lastVisibleLineMapping = this.lineWordMappings[this.visibleEndIndex];

            if (!firstVisibleLineMapping || !lastVisibleLineMapping) return;

            const firstWordIndex = firstVisibleLineMapping.start;
            const lastWordIndex = lastVisibleLineMapping.end;

            for (let i = firstWordIndex; i <= lastWordIndex; i++) {
                const wordElement = this.wordElements[i];
                if (wordElement && wordElement.offsetParent) {
                    const charSpans = wordElement.childNodes;
                    charSpans.forEach(charNode => {
                        if (charNode.nodeType === Node.ELEMENT_NODE) {
                            const charSpan = charNode as HTMLElement;
                            const charRect = charSpan.getBoundingClientRect();
                            const charCenter = (charRect.left - containerRect.left) + charRect.width / 2;
                            
                            const targetColor = (charCenter >= leftMarginEnd && charCenter <= rightMarginStart)
                                ? activeColor
                                : inactiveColor;

                            if (charSpan.style.color !== targetColor) {
                                charSpan.style.color = targetColor;
                            }
                        }
                    });
                }
            }
        } else if (displayMode === 'Fixation') {
            // Colors are applied statically in createLineElement for Fixation mode.
            // No dynamic highlighting is needed here.
        }
    }
